...
public class ECommerceApp {
    public static void main(String[] args) {
        ...
        products.stream() // 1 Punkt
                .filter(p -> p.stock() == 0) // 1 Punkt
                .sorted(Comparator.comparingDouble(Product::price).reversed()) // 1 Punkt
                .forEach(System.out::println); // 1 Punkt

        String allNames = "";
        allNames = products.stream() // 1 Punkt
                .map(Product::name) // 1 Punkt
                .collect(Collectors.joining(", ")); // 1 Punkt
        System.out.println("\n--- All Products ---\n" + allNames);
    }
}
